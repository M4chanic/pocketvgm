...
pub struct Clocks {
    ...
    pub fds: bool,
    /// Бит 31 поля YM2413: чип — VRC7 (Konami, NES): 6 каналов, свой
    /// набор патчей, ритма нет
    pub vrc7: bool,
    /// Разновидность шума SN76489: маска отводов (поле 0x28) и ширина
    /// сдвигового регистра (поле 0x2A). У Master System это 0x0009 и 16
    /// бит, у SN76489AN (SG-1000, аркады, BBC Micro) — 0x0003 и 15, и шум
    /// у них звучит по-разному и по высоте, и по окраске. Ноль в поле
    /// означает «по умолчанию», то есть вариант Master System.
    pub sn_feedback: u16,
    pub sn_sr_width: u8,
...
impl Header {
    pub fn parse(d: &[u8]) -> Result<Header, Error> {
        ...
        let clocks = Clocks {
            ...
            vrc7: rd32(d, 0x10).unwrap_or(0) & 0x8000_0000 != 0,
            sn_feedback: if hdr_end > 0x29 {
                let v = d[0x28] as u16 | (d[0x29] as u16) << 8;
                if v == 0 { 0x0009 } else { v }
            } else {
                0x0009
            },
            sn_sr_width: if hdr_end > 0x2A && d[0x2A] != 0 { d[0x2A] } else { 16 },
