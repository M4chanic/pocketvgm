...
fn vgm_play(staged: &'static [u8], pl: &PlayCtx) -> Ctl {
    ...
    chipbox_write(
        0x15,
        // В файлах Mega Drive гейн PSG 39, а не 33. Относительно FM наш
        // PSG был тише НАСТОЯЩЕЙ приставки на 2.73 дБ: замер тоновым
        // сигналом MDFourier (scripts/mdfourier_vgm.py) против записи
        // японской Mega Drive I ревизии VA1 из базы MDFourier. Причём
        ...
        //
        // Поправка разделена пополам: PSG вверх на 1.45 дБ (33 -> 39) и FM
        // вниз на 1.25 дБ (239 -> 207, ниже по этому же вызову).
        // Соотношение сдвигается на нужные 2.70 дБ, а сумма не растёт —
        // поднимать её нельзя, у Streets of Rage 2 пик и без того стоял
        // на 99.8% шкалы.
        //
        ...
        gain_of(header, if sn_clk != 0 { if fm_clk != 0 { 39 } else { 33 } } else { 0 }, 0x00) << 8
