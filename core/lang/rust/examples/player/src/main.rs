...
/// регистре восьмибитный, и просьбу «в 64 раза громче» (спецификация это
/// допускает) выполнить всё равно нечем.
/// Понижение группы Mega Drive на файлах 32X.
///
/// Сумма Mega Drive и без того стоит у потолка (у Streets of Rage 2 пик в
/// 0.02 дБ от шкалы), а на 32X к ней добавляется PWM. На After Burner
/// «Maximum Power» сумма с PWM на его законном уровне выходила за шкалу
/// на 1.64 дБ. Опускаем ВСЮ группу на четверть, вместе с самим PWM:
/// отношения чипов внутри файла сохраняются, а файл просто играет на
/// 2.5 дБ тише — разброс между рипами и так двадцать с лишним дБ.
fn md_scale(v: u32) -> u32 {
    if PWM_FILE.load(core::sync::atomic::Ordering::Relaxed) { v * 3 / 4 } else { v }
}

/// В текущем файле объявлен PWM (32X)
static PWM_FILE: core::sync::atomic::AtomicBool = core::sync::atomic::AtomicBool::new(false);
...
fn vgm_play(staged: &'static [u8], pl: &PlayCtx) -> Ctl {
    ...
    // Признак файла 32X: по нему md_scale опускает всю группу
    PWM_FILE.store(header.clocks.pwm != 0, core::sync::atomic::Ordering::Relaxed);
    if ym_clk == 0
        && ay_clk == 0
        && pcm_clk == 0
        && adpcm_clk == 0
    ...
    if huc_clk != 0 {
        ...
        //
        // 150, а не 180 (2026-08-05). На 180 рипы с CD-приставки упирались
        // в шкалу — те самые, на которые и была жалоба. Замер стенда ключом
        // --huc-gain, пик самой музыки пересчитан на 180:
        //     Ys IV      +0.69 дБ FS   предельный гейн 166
        //     Garou 2    +0.01                        180
        //     Snatcher   -1.98                        226
        //     Xanadu II  -6.71                        390
        // На 150 самый тесный из них ложится на -0.9 дБ FS.
        //
        // Разброс между играми при этом НЕ наш: у эталона Ys IV на 3.5 дБ
        // громче Batman, то есть CD-рипы громче сами по себе. Наш вклад
        // после правки кривой — около 1.1 дБ (картриджные +2.1, CD +3.0).
        //
        // 180, а не 228 (2026-08-04). Прежнее число выводилось при КРУТОЙ
        // кривой громкости чипа: она была 1.5 дБ на шаг вместо эталонных
        // 1.0869, и всё, что играет не на полной громкости, выходило тише
        // — на середине шкалы на 6 дБ. Множитель это компенсировал, а
        // рипы, держащие громкость у максимума, компенсации не требовали
        // и выбивались вверх (задача 86). После правки кривой те же файлы
        // на 228 стали упираться в шкалу: Batman и Soldier Blade дали
        // пик 0.00 дБ FS. На 180 пик уходит на -2 дБ, а медианы
        // (-17.7, -18.2, -20.1) остаются в полосе Mega Drive.
        //
        ...
        chipbox_write(0x28, gain_of(header, 150, 0x1B));
    }
    ...
    chipbox_write(
        ...
        gain_of(header, if sn_clk != 0 { if fm_clk != 0 { md_scale(36) } else { 33 } } else { 0 }, 0x00) << 8
            ...
            | gain_of(header, if fm_clk != 0 { md_scale(207) } else if opn_clk != 0 { 204 } else { 0 }, fm_id),
    );
    ...
    let gains = gain_of(header, if adpcm_clk != 0 { 64 } else { 0 }, 0x17) << 24
        ...
        // Отдельный AY — 64, SSG внутри OPN — 47 (2026-08-05).
        //
        // Раньше стояло 64 и там, и там: перекос FM против SSG выправляли
        // подъёмом FM, см. комментарий выше. После правки кривой громкости
        // AY (задача 90) отношение перемерено раздельным глушением FM и
        // SSG на двух рипах YM2203, и оно всё ещё короче эталонного:
        //     Dailyopening   эталон +14.60, наш +11.04  -> -3.56 дБ
        //     Office         эталон  -1.15, наш  -3.09  -> -1.94 дБ
        // Медиана 2.75 дБ, отсюда 64 / 10^(2.75/20) = 47. Правка служит
        // той же цели, что прошлая калибровка: свести отношение, а не
        // абсолютный уровень.
        //
        // Врозь потому, что множитель на обе дороги один, а отдельный AY
        // после правки кривой сошёлся с эталоном в пределах 0.11 дБ —
        // трогать его нельзя. libvgm делит SSG составного чипа пополам
        // ровно по этой же причине.
        | gain_of(header, if ay_clk != 0 { 64 } else if opn_clk != 0 { 47 } else { 0 }, ssg_id) << 8
    ...
    //
    // 86 = 114 x 0.75, где 114 — замер, а 0.75 — общее понижение группы
    // 32X, см. md_scale.
    //
    // Замер по методу задачи 69 (синтетический файл, где сначала звучит
    // тон PSG, потом меандр PWM в половину шкалы: нормировка эталона в
    // отношении сокращается):
    //     эталон  PSG -18.12 дБ, PWM  -1.28 -> отношение 16.84 дБ
    //     наше    PSG -17.87 дБ, PWM  -6.06 -> отношение 11.81 дБ
    // Поправка 5.04 дБ, то есть 64 -> 114. Наш PWM при 64 выдаёт ровно
    // расчётные -6.02 дБ, так что расходится не наш чип, а таблица
    // громкостей эталона: libvgm делит SN76489 пополам (0x80), а PWM
    // почти не трогает (0xE0).
    //
    // Если целиться не в PSG, а в FM, число выходит 149: наш PSG на
    // 2.31 дБ тише libvgm ОТНОСИТЕЛЬНО FM, потому что баланс PSG/FM мы
    // свели по медиане парка настоящих Mega Drive, а не по эталону (см.
    // память libvgm-mixes-psg-wrong). Взято меньшее: разница между двумя
    // якорями и есть та неопределённость, которую снимет только запись с
    // настоящей 32X.
    chipbox_write(0x37, if header.clocks.pwm != 0 { gain_of(header, 86, 0x11) } else { 0 });
    ...
    chipbox_write(
        ...
        gain_of(header, if opll { OPLL_GAIN } else if opl_clk != 0 { 16 } else { 0 }, opl_id) << 24
            ...
            //     громкость 12  -4.66       громкость 1  -4.55
            //     форма: эталон -1.94/-5.46/-11.48/-23.52,
            //            наш    -1.94/-5.46/-11.47/-23.42
            // То есть модель чипа верна, расходился только множитель.
            //
            // Запаса хватает: у чипа максимум четырёх каналов при NR50=7
            // даёт размах 60*7*64 = 26880, то есть амплитуду 13440 и
            // -7.7 дБ FS. После подъёма это -3.1 дБ, и упереться не может
            // даже теоретический максимум.
            //
