...
fn vgm_play(staged: &'static [u8], pl: &PlayCtx) -> Ctl {
    ...
    chipbox_write(
        0x15,
        gain_of(header, if sn_clk != 0 { 33 } else { 0 }, 0x00) << 8
            ...
            // упереться в потолок. Опасение оказалось устаревшим: пик на
            // громком треке 4538 из 32767, то есть до потолка 17 дБ.
            // Микшер с той оценки успел смениться дважды.
            //
            ...
            | gain_of(header, if fm_clk != 0 { 239 } else if opn_clk != 0 { 204 } else { 0 }, fm_id),
    );
