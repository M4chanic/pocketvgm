...
`endif
    ...
    // Снимаем подтверждение только когда чип снял запрос: это и есть
    // признак, что он его увидел. Держать один шаг cen_nes оказалось
    // мало — APU защёлкивает ответ в своей фазе (aclk1_d), и выборок
    // выходило 56 из 241.
    if (apu_dma_ack && !apu_dma_req) apu_dma_ack <= 0;
