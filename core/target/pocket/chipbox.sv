...
`endif
  ...
  // Затравка блокеров после сброса.
  //
  // Выход этих чипов однополярный и сразу после сброса стоит на заметной
  // постоянной. Блокер считает y[n] = x[n] - x[n-1] + y[n-1]*(1-2^-11), а
  // x[n-1] после сброса равен нулю (или, при смене трека, значению от
  // прежнего файла). Первая же разность вносит полный скачок, который
  // потом вытекает сто миллисекунд: у Mr. Gimmick это 338 отсчётов в
  // упоре на 0.019 с, то есть щелчок в начале каждого трека.
  //
  // На первом подотсчёте после сброса разности не делаем вовсе: просто
  // запоминаем вход и держим выход в нуле.
  reg hp_prime = 1;
...
`endif
  ...
  always @(posedge clk) begin
    if (sub_tick) begin
      ...
      sub_phase <= ~sub_phase;
      if (hp_prime) begin
        // Затравка блокеров, см. объявление hp_prime: запоминаем вход и
        // держим выход в нуле, чтобы первой разности не было вовсе.
        hp_prime <= 0;
        ay_hp_x <= ay_wide;
        ay_hp_y <= 0;
        apu_hp_x <= apu_wide;
...
`endif
    ...
    if (soft_reset_req) begin
      gb_beat <= 0;
      opl_beat <= 0;
      // Смена трека — тоже сброс: чипы уходят в тишину, а в x[n-1] лежит
      // постоянная прежнего файла. Без затравки это второй такой же
      // щелчок, теперь уже в обратную сторону.
      hp_prime <= 1;
    end
