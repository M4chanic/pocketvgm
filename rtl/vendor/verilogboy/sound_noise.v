...
module sound_noise(
    ...
    // m4pocket: при r = 0 делитель был вдвое короче, чем нужно.
    //
    // Эталон (gbsplay, gbhw.c): div_tc = 16 << s, дальше умножить на r,
    // а при r = 0 — поделить на 2. То есть период равен 16*r << s, и
    // ровно 8 << s при нуле. Здесь LFSR тактируется каждые 2^(s+1)
    // периодов делителя, значит период делителя должен быть 8*r тактов,
    // а при r = 0 — четыре. Стояло adjusted = 1, то есть два.
    //
    // Померено на тактовой железа, узкий 7-битный режим, период
    // последовательности снят автокорреляцией:
    //     r=0 s=4: эталон 32768 Гц, было 65548 Гц (ровно октава вверх)
    //     r=1 s=4: эталон 16384 Гц, было 16387 Гц (верно)
    // r = 0 — самая ходовая настройка для ярких ударных, поэтому вверх
    // уезжали хэты и малый барабан во ВСЕХ файлах Game Boy.
    always @(posedge clk)
    begin
        if (start_posedge) begin
            adjusted_freq_dividing_ratio <=
                    (freq_dividing_ratio == 3'b0) ? (6'd3) : ((freq_dividing_ratio * 8) - 1);
